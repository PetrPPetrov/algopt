# AlgOpt code style convention

## Language standard
On Windows we use the latest Visual Studio IDE (currently VS 2022) and the corresponding C++ compiler.

## File names
* Source code should be stored in files either with *.cpp*, *.h* or *.hpp* extensions.
...
* Header files should be separated into declaration files (*.h*) and implementation files (*.hpp*).
  Declarations (class/struct definitions, function declarations, etc.) should be placed in *.h* files.
  Implementations (method bodies, function definitions) should be placed in corresponding *.hpp* files.
  The *.hpp* files should be included from *.cpp* files that need the implementations, not from *.h* files.
  All methods and functions in *.hpp* files should be marked with *inline* keyword.
...
* Include Gkm-Local specific libraries, like ```#include "protocol.h"```
...
* Class and structure fields should use *snake_case* naming convention starting without any prefix.
  For instance, *buffer*. If field name conflicts with argument name then add *'_arg'* suffix to the argument name,
  for instance, *buffer_arg*.
